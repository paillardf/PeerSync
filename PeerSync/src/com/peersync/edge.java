...
public class edge {
	...
	public static final String EdgeId = "59616261646162614E504720503250337A14A255564B20BDA08CFB53FB0CDD2203";
	...
	public static final File CertificateFile = new File(CertificateDirectory,"59616261646162614E50472050325033B1C66B1C64A767E0C896C8F2E708C9D903" +".crt");
	...
	public static void main(String[] args) {
		...
		//JxtaSocket SocketClient = null;

		System.out.println("Step 0 : Setting the PID + directories creation if needed");
		try {
			PID = (PeerID) IDFactory.fromURI(URI.create("urn:jxta:cbid-"+EdgeId));
		...
		try {
			MyNetworkManager = new NetworkManager(NetworkManager.ConfigMode.EDGE, Name, ConfigurationFile.toURI());
			...
			if( ! MyNetworkConfigurator.exists()) {
				String TheSeed = "tcp://127.0.0.1:" + rdvsecure.TcpPort;
		...
		MyNetworkPeerGroup.getRendezVousService().setAutoStart(false);

		System.out.println("\t connecting to Rendez-vous");
		if (MyNetworkManager.waitForRendezvousConnection(120000))
			System.out.println("\tSuccessfully connected to Rendez-Vous");
		else {
			System.out.println("\tFailed to connect to Rendez-Voux peer; Aborting");
			System.exit(1);
		}
		...
		try {
			...
			while(i>0) {
				...
				PeerID loadedPeer = (PeerID) IDFactory.fromURI(URI.create("urn:jxta:cbid-"+rdvsecure.getFilenameWithoutExtension(CRTfiles[i].getName())));
				if(!loadedPeer.equals(PID)) {
					System.out.println("\tAdding PeerID to trusted list : "+rdvsecure.getFilenameWithoutExtension(CRTfiles[i].getName()));

					ByteArrayInputStream bais = new ByteArrayInputStream(rdvsecure.getBytesFromFile(CRTfiles[i]).toByteArray());
		...
		System.out.println("Step 4 : Setting up the socket");


		try {
			// Retrieving the output stream
			OutputPipe outputPile =  MyNetworkPeerGroup.getPipeService().createOutputPipe(rdvsecure.GetPipeAdvertisement(), 2000);

			//			tputStream MyOutputStream = SocketClient.getOutputStream();
			//DataOutput MyDataOutput = new DataOutputStream(MyOutputStream);

			// Sending a message
			outputPile.send(new Message());

			//InputStream MyInputStream = SocketClient.getInputStream();
			//DataInput MyDataInput = new DataInputStream(MyInputStream);
			//
			//String IncomingMessage = MyDataInput.readUTF();
			//System.out.println(Name + " - Received socket message:\n\n" + IncomingMessage);

		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("Phase 4 : Shutting down the jxta network");
